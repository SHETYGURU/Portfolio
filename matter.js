...
function runMatter() {
  ...
  var attractiveBody = Bodies.circle(
    ...
    Math.max(dimensions.width / 50, dimensions.height / 50) / 8, // Smaller radius
    {
      render: {
        fillStyle: `#000`,
        strokeStyle: `#000`,
      ...
      plugin: {
        attractors: [
          function (bodyA, bodyB) {
            ...
            // Avoid strong influence within a certain radius
            if (distance < 30) return { x: 0, y: 0 }; // No attraction for very close bodies
  ...
  Events.on(engine, "afterUpdate", function () {
    ...
    Body.translate(attractiveBody, {
      x: (mouse.position.x - attractiveBody.position.x) * 0.05, // Lower speed for smoother transitions
